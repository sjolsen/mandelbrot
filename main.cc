...
{
	if (n < 256)
		return pixel {0, 0, n & 0xFF};
	else if (n < 512)
		return pixel {(n & 0xFF) >> 1, 0, 0xFF};
	else if (n < 768)
		return pixel {0x80 & ((n & 0xFF) >> 1), n & 0xFF, 0xFF};
	else
		return pixel {0xFF, 0xFF, ~(n & 0xFF)};
}
          ...
          char** argv)
{
	...
	if (cudaMalloc (static_cast <void**> (GPU_escape_times), image_height * image_width * sizeof (uint32_t)) != cudaSuccess)
	{
	...
	for (int row = 0; row < image_height; ++i)
		transform (times [i], times [i] + image_width, picture [i], [] (uint32_t n) { return colorize (n); });

	// Write to file

	ofstream bigger ("biggercuda.bmp");
	write_data_to_stream (picture, bigger);
}
